...
template <class T>
class GavnoTree : public ITree<T> {
private:
...
public:
    ...
    GavnoTree(InitIterator begin, InitIterator end) : GavnoTree() {
        for (InitIterator cur(begin); cur != end; ++cur) {
            Insert(*cur);
        }
    }

    GavnoTree(std::initializer_list<T> list) : GavnoTree() {
        for (const T& value : list) {
            Insert(value);
        }
    }

    GavnoTree(const GavnoTree& other) : GavnoTree() {
        for (const T& value : other) {
            Insert(value);
        }
    }

    GavnoTree(GavnoTree&& other) noexcept : GavnoTree() {
    ...
    GavnoTree(std::shared_ptr<ITree<T>> other)
        : GavnoTree(*dynamic_cast<GavnoTree<T>*>(other.get())) {
    }

    GavnoTree& operator=(const GavnoTree& other) {
    ...
    GavnoTree& operator=(GavnoTree&& other) noexcept {
        if (root_ == other.root_) {
            return *this;
        }
        std::swap(root_, other.root_);
    ...
    std::shared_ptr<BaseImpl> Find(const T& value) const override {
        ...
        return const_cast<GavnoTree<T>*>(this)->FindImpl(root_, val);
    }

    std::shared_ptr<BaseImpl> LowerBound(const T& value) const override {
        std::optional<T> val(value);
        return const_cast<GavnoTree<T>*>(this)->LowerBoundImpl(root_, val);
    }
...
private:
    ...
    class GavnoTreeItImpl : public BaseImpl {
    private:
        std::shared_ptr<Node> it_;

    public:
        GavnoTreeItImpl() = delete;

        explicit GavnoTreeItImpl(std::shared_ptr<Node> ptr) : it_(ptr) {
        }

        GavnoTreeItImpl(const GavnoTreeItImpl& other) : it_(other.it_) {
        }

        std::shared_ptr<BaseImpl> Clone() const override {
            return std::make_shared<GavnoTreeItImpl>(*this);
        }
        ...
        bool IsEqual(std::shared_ptr<BaseImpl> other) const override {
            auto casted = std::dynamic_pointer_cast<GavnoTreeItImpl>(other);
            if (!casted) {
                return false;
            }
    ...
    std::shared_ptr<BaseImpl> Begin() const override {
        return std::make_shared<GavnoTreeItImpl>(begin_);
    }

    std::shared_ptr<BaseImpl> End() const override {
        return std::make_shared<GavnoTreeItImpl>(end_);
    }

    std::shared_ptr<BaseImpl> FindImpl(std::shared_ptr<Node> from, const std::optional<T>& value) {
        ...
        while (true) {
            ...
            if (value < cur_node->value_) {
                ...
                Splay(cur_node);
                return std::make_shared<GavnoTreeItImpl>(root_);
            }
        }
    }
    ...
    std::shared_ptr<BaseImpl> LowerBoundImpl(std::shared_ptr<Node> from,
        ...
        if (value < from->value_) {
            if (from->left_) {
                ...
                return std::make_shared<GavnoTreeItImpl>(from);
            }
            ...
            if (from->right_) {
                ...
                Splay(from);
                auto tmp = std::make_shared<GavnoTreeItImpl>(from);
                tmp->Increment();
                return tmp;
            }
        } else {
